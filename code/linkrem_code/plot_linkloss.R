## Function to generate the figure meanlambda ~ lrem for different loss scenarios
...
plot_linkloss <- function(data, webs, paramss, scenarioss, spfs, a, b, basals, ptitle, labnames){
  ...
  data <- data %>% filter(params %in% paramss) %>% 
    mutate(scenario=factor(scenario, levels=c("best-case scenario", "worst-case scenario", "random scenario")), 
        ...
        #params=factor(params,levels=c(paramss[1],paramss[2])))
  ...
  data %>% filter(web == webs, params %in% paramss, spf==spfs, alpha == a, beta == b, scenario %in% scenarioss) %>%
    group_by(web, params, scenario, lrem, ITL) %>%
    summarise(meanlambda=mean(lambda,na.rm=TRUE), sdlambda=sd(lambda, na.rm=TRUE)) %>% ungroup %>% 
    rename(`Fraction of links removed`=lrem) %>%
    ggplot() +
    aes(x=`Fraction of links removed`, y=meanlambda, colour=as.factor(ITL), fill=as.factor(ITL)) +
    scale_color_manual(values=cpalette) +
    scale_fill_manual(values=cpalette) +
    geom_hline(yintercept=1, linetype="dashed", alpha=0.3) +
    geom_vline(data = topextinct, aes(xintercept = minlrem), linetype="dotdash", alpha=0.7) +
